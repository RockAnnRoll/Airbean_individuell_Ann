...
router.post("/menu", requireAdmin, async (req, res) => {
  ...
  if (!id || !title || !desc || !price) {
    return res
      .status(400)
      .send("All fields (id, title, desc, price) are required");
  }
  const newItem = {
    id,
    title,
...
router.post("/campaign", requireAdmin, async (req, res) => {
  ...
  const invalidProducts = products.filter(
    (product) => !menuIds.includes(product)
  );
  ...
  if (invalidProducts.length > 0) {
    return res
      .status(400)
      .send(
        `The following products do not exist in the menu: ${invalidProducts.join(
          ", "
        )}`
      );
  }
  ...
  res
    .status(201)
    .json({ message: "Campaign added successfully", addedCampaign });
});
